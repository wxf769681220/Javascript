<template>
  <div class="api-4">
    <Card shadow style="width:1200px">
      ...
      <div>
        <ul class="menu">
          <li>页面性能一直都是 Web 开发人员比较关注的领域。但在实际应用中，度量页面性能的指标，是 javascript 的 Date 对象。</li>
          <li>Web Timing API 改变了这个局面，让开发人员通过 javascript 就能使用浏览器内部的度量结果，给出了页面加载和渲染过程的很多信息，对性能优化非常有价值。</li>
          <li>Web计时机制的核心是 window.performance 对象。</li>
          <li>performance 对象的属性：navigation、timing、memory 等。</li>
        </ul>
        <div v-highlight>
          <pre>
            <code>
    ...
    <Card shadow style="width:900px">
      ...
      <div>
        <p>performance 对象还包括两个方法：getEntries() 和 now() 两个方法。</p>
        <div v-highlight>
          <pre>
            <code>
              // getEntries()方法将返回一个数组
              // 包含了页面中所有的HTTP资源请求
