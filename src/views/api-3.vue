  ...
  <div class="api-3">
    <div class="layout-content">
      <Card dis-hover shadow style="width:620px">
        ...
        <div>
          <p>File API为客户端访问计算机中的文件，并更好地对这些文件执行操作提供了一种安全的方式。</p>
          <div v-highlight>
            <pre>
              <code>
                /* html结构：
      ...
      <Card dis-hover shadow style="width:800px">
        ...
        <div>
          <p>FileReader类型实现的是一种异步文件读取机制。它与XMLHttpRequest很像，区别是它读取的文件系统，而不是远程服务器。FileReader类型提供以下几个方法：</p>
          <ul class="menu">
            <li>readAsText(file, encoding)：以纯文本形式读取文件，将读取到的文本保存在result属性中。第二个参数用于指定编码类型（可选）。</li>
            <li>readAsDataURL(file)：读取文件并将文件以数据URI的形式保存在result属性中。</li>
            <li>readAsBinaryString(file)：读取文件并将一个字符串保存在result中，字符串中的每个字符表示一个字节。</li>
          ...
          <p>由于读取过程是异步的，因此FileReader也提供了以下几个事件：</p>
          <ul class="menu">
          ...
          <p>
            若想中断读取过程，则可以使用abort()方法，这样就会触发以个abort()事件。在触发load、error或abort()事件之后，会触发另一个事件loadend事件，loadend事件发生意味着已经读取完整个文件，
            或读取时发生了错误，或读取过程被中断。
          </p>
          <div v-highlight>
            <pre>
      ...
      <Card dis-hover shadow style="width:650px">
        ...
        <div>
          <p>
            File对象支持一个slice()方法，在firefox中的实现叫mozSlice()，在chrome中的实现叫webkitSlice()。slice()方法接收两个参数：起始字节和要读取的字节数。
            这个方法返回一个Blob的实例，Blob是File类型的父类型。
          </p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:680px">
        ...
        <div>
          <p>blob URL：指的是引用保存在file或blob中数据的URL，它不必把文件内容读取到javascript而直接使用文件内容。为此，只要在需要文件内容的地方提供blob URL即可。</p>
          <p>创建blob URL，可以使用window.URL.createObjectURL()方法，并传入File对象或blob对象。chrome中的实现叫window.webkitURL.createObjectURL()。</p>
          <div v-highlight>
            <pre>
              <code>
                //在不同实现中创建blob URL
      ...
      <Card dis-hover shadow style="width:700px">
        ...
        <div>
          <p>
            结合HTML5提供的拖放API。将一张图片、一个连接或一个文件，从桌面拖放到浏览器中会触发拖放事件的drop事件，并且可以在event.dataTransfer.files中
            读取到被放置文件，此时它就是一个File对象。
          </p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:740px">
        ...
        <div>
          <p>通过File API可以获取文件内容，把文件内容方到send()方法，再通过POST请求，就能实现上传。以上做法只能上传的是文件内容，服务器需要收集内容，再把它保存到另一个文件中。</p>
          <p>通过FormData类型，我们可以将文件像表单一样提交给服务器。</p>
          <div v-highlight>
            <pre>
              <code>
                var filesList = document.getElementById("myFile")
