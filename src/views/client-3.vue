  ...
  <div class="client-3">
    <div class="layout-content">
      <Card dis-hover shadow style="width:580px">
        ...
        <p>浏览器发送http请求，服务器接收请求并做出响应，浏览器引擎获取服务器响应文本。渲染引擎在取得内容之后的基本流程：</p>
        <div>
          <ol class="menu">
            <li>解析html文档 - 构建DOM树；</li>
            <li>解析css - 构建CSSOM树；</li>
            <li>将DOM树与CSSOM树合并生成用于render tree（渲染树）；</li>
            <li>渲染引擎遍历render tree，然后对网页进行布局与绘制，最后渲染出来；</li>
            <li>若DOM树发生改变，渲染引擎会重新遍历render tree，然后对网页进行重绘；</li>
          </ol>
          <h4 style="margin-top: 10px">在上述过程中，会同时进行网络资源的加载，如：css样式表、js脚本或图像。</h4>

          <h4 style="margin: 10px 0">在解析html文档时，若遇见&lt;link>标签，</h4>
          <ol class="menu">
            <li>浏览器会向服务器发送请求资源。在资源下载的过程中，不会阻塞对html的解析。</li>
            <li>若请求的资源是css样式表，render树的渲染。</li>
            <li>若请求的资源是图像，图像加载完毕后，不会直接渲染出来，它会等到Render Tree完成后一起渲染出来。</li>
          </ol>

          <h4 style="margin: 10px 0">在解析html文档时，若遇见&lt;script>标签，</h4>
          <ol class="menu">
            <li>遇到script标签，对html的解析将会停下来，然后去执行js代码，因此script标签通常建议放在&lt;/body>之前，能优化用户体验，减少白屏时间。</li>
          </ol>
        </div>
      </Card>
      <Card dis-hover shadow style="width:1000px">
        <h3 slot="title">2.浏览器缓存</h3>
        <p>浏览器根据第一次http请求返回的响应头部信息来确定该资源要不要缓存，如何去缓存。我们将缓存过程分为强缓存和协商缓存。</p>
        <h4 style="margin: 10px 0">强缓存：</h4>
        <div>
          <ol class="menu">
            <li>强缓存：不会向服务器重新发送请求，直接从缓存中读取资源。</li>
            <li>
              强缓存是通过设置Web服务器响应头（HTTP Header）字段实现的：Expires 和 Cache-Control
              <ol class="menu">
          ...
          <ol class="menu">
            <li>协商缓存：强缓存失效后，浏览器向服务器发起请求时，由服务器根据缓存标识决定是否使用缓存。协商缓存生效，http状态码会返回304（Not modified）；
              协商缓存失效，http状态码会返回200。</li>
            <li>
              协商缓存是通过设置Web服务器响应头（HTTP Header）字段实现的：Last-Modified 和 ETag
              <ol class="menu">
          ...
          <ol class="menu">
            <li>Service works：运行在浏览器背后的独立线程，一般可以用来实现缓存功能。</li>
