  ...
  <div class="api-2">
    <div class="layout-content">
      <Card dis-hover shadow style="width:400px">
        ...
        <div>
          <p>Geolocation API在浏览器中的实现是navigator.geolocation对象，该对象包含以下三个方法：getCurrentPositon()、watchPosition()和clearWatch()。</p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:850px">
        ...
        <div>
          <p>getCurrentPositon()：调用此方法会触发请求用户共享地理定位信息的对话框。</p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:650px">
        ...
        <div>
          <p>watchPosition()方法：该方法接收的参数与getCurrentPositon()方法完全相同。实际上该方法与定时调用getCurrentPositon()方法的效果是一样的。
            在第一次调用watchPosition()方法后会取得当前位置，执行成功回调函数与错误回调函数。然后watchPosition()就地等待系统发出位置已改变的信号（它不会自己轮询位置）。</p>
          <div v-highlight>
            <pre>
              <code>
                var Pos = navigator.geolocation
