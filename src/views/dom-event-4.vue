<template>
  <div class="dom-event-4">
    <div class="layout-content">
      <Card dis-hover shadow style="width:680px">
        <h3 slot="title">1.orientationchange事件</h3>
        <div>
          <p>
            苹果公司为移动safari中添加orientationchange事件，可以让开发人员确定用户何时将设备由横向查看模式切换为纵向查看模式。window.orientation属性中可能包含三个值：
            0：表示肖像模式；90：表示左旋转横向模式；-90：表示右旋转横向模式，（180：表示iphone头朝下）
          </p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:680px">
        ...
        <div>
          <p>firefox为移动设备添加MozOrientation事件，当设备的加速针加测到设备方向改变时会触发这个事件。</p>
          <div v-highlight>
            <pre>
              <code>
                //Moz为特定浏览器前缀
      ...
      <Card dis-hover shadow style="width:780px">
        ...
        <div>
          <p>
            设备上的加速针检测到设备方向发生变化时在window上触发。它的意图是告诉开发人员设备在空间中朝向哪儿，而不是移动。
            设备在空间中靠x、y、z轴来确定。触发deviceorientation事件时，事件对象包含着每个轴相对于设备静止状态下发生变化的信息。
          </p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:780px">
        ...
        <div>
          <p>devicemotion事件可以告诉开发人员设备是什么时候移动，而不仅仅是设备的方向发生变化。devicemotion事件能够检测到设备是不是正在往下掉。或者是不是被走着的人拿在手里。</p>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:1100px">
        ...
        <div>
          ...
          <ul class="menu">
            ...
            <li>touchmove：当手指在屏幕上滑动时连续触发。（若此操作是在滚动页面，则可以用preventDefault()来阻止滚动）</li>
          ...
          <div v-highlight>
            <pre>
              <code>
                //以下事件都会冒泡，也可以取消
                EventUtil.addHandler(window, "touchstart", function (event) {
                  event = EventUtil.getEvent(event)
      ...
      <Card dis-hover shadow style="width:980px">
