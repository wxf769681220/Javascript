<template>
  <div class="page-1">
    <Card shadow style="width:650px">
      ...
      <div>
        <ul class="menu">
          <li>有时候，开发者需要知道，用户正在离开页面。常用的方法是监听以下三个事件：pagehide、beforeunload 和 unload。</li>
          <li>
          ...
          <li>不管手机或桌面电脑，这个 API 都会监听到页面的可见性发生变化。</li>
          <li>
    ...
    <Card shadow style="width:750px">
      ...
      <div>
        <p>在 document 对象上，新增了一个 document.visibilityState 属性。该属性返回一个字符串，表示页面当前的可见性状态，共有三个可能的值：</p>
        <ul class="menu">
        ...
        <p>
          只要 document.visibilityState 属性发生变化，就会触发 visibilitychange 事件。因此，可以通过监听这个事件（通过document.addEventListener()方法或 document.onvisibilitychange 属性），
          跟踪页面可见性的变化。
        </p>
        <div v-highlight>
          <pre>
    ...
    <Card shadow style="width:740px">
      ...
      <div>
        ...
        <p>以上这些情况，都会触发 visibilitychange 事件。由此可见，visibilitychange 事件比 pagehide、beforeunload、unload 事件更可靠。因此，优先选择使用 visibilitychange 事件。</p>
        <div v-highlight>
          <pre>
            <code>
              /*
