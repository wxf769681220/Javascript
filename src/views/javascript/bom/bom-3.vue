<template>
  <div class="bom-3">
    ...
    <Card shadow style="width:780px">
      <h3 slot="title">2.navigator.cookieEnabled 和 navigator.userAgent</h3>
      <div>
    ...
    <Card shadow style="width:700px">
      ...
      <div>
        <p>navigator.registerConentHandler() 和 navigator.registerProtocolHandler() 方法，可以让一个站点指明它可以处理特定类型的信息。
          随着 RSS 阅读器和在线电子邮件程序的兴起，注册处理程序就为像使用桌面应用程序一样默认使用这些在线应用程序提供了一种方式。
        </p>
        <p>navigator.registerConentHandler() 方法，它接收三个参数：要处理的 MIME 类型、可以处理该 MIME 类型的页面的 URL 及应用程序名称。</p>
        <div v-highlight>
          <pre>
            <code>
        ...
        <p>navigator.navigator.registerProtocolHandler() 方法，它接收三个参数：要处理的协议（ru :mailto或ftp）、处理该协议的页面的 URL 及应用程序名称。</p>
        <div v-highlight>
          <pre>
            <code>
              // 要将一个应用程序注册为默认的邮件客户端
    ...
    <Card shadow style="width:850px">
      ...
      <div>
        <p>以下是完整的用户代理字符串检测脚本，包括检测呈现引擎、平台、Windows 操作系统、移动设备和游戏系统。</p>
        <div v-highlight>
          <pre>
            <code>
              var client = function () {
