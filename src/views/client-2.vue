  ...
  <div class="client-2">
    <div class="layout-content">
      <Card dis-hover shadow style="width:480px">
        ...
        <div>
          <ul class="menu">
            ...
            <li>URIError：在使用encodeURI()或decodeURI()，而URI格式不正确。</li>
            <li>RangeError：数值超出相应范围错误。</li>
            <li>EvalError：数值超出相应范围错误。</li>
          </ul>
          <div v-highlight>
            <pre>
              <code>
      ...
      <Card dis-hover shadow style="width:480px">
        ...
        <div>
          <p>try/catch/finally语句作为Javascript中处理异常的一种标准机制。</p>
          <div v-highlight>
            <pre>
              <code>
                try {
      ...
      <Card dis-hover shadow style="width:560px">
        ...
        <div>
          <ul class="menu">
            <li>任何没有通过try-catch处理的错误都会触发windwo对象的error事件。</li>
            <li>在任何web浏览器中。error事件处理程序都不会创建event事件对象，但它可以接收三个参数：错误消息、错误所在的URL和错误行号。</li>
            <li>要指定onerror事件处理程序，必须使用DOM0级，因为它没有遵循DOM2级的标准格式。</li>
          </ul>
          <div v-highlight>
            <pre>
              <code>
          ...
          <p>图像也支持onerror事件。若图像src特性中的URL不能返回有效的图像格式，就会触发error事件。并且此时的error事件会遵循DOM格式，会返回一个以图像为目标的event对象。</p>
          <div v-highlight>
            <pre>
              <code>
                var imgs = new Image();
      ...
      <Card dis-hover shadow style="width:560px">
        ...
        <div>
          <p>要建立这样一种javascript错误记录系统，首先需要在服务器上创建一个页面或服务器入口点，用于处理错误数据。这个页面无非就是从查询字符串中取得数据，然后将数据写入错误日志中。</p>
          <div v-highlight>
            <pre>
              <code>
                //使用图像PING技术
